...
const AddTodo = ({ onAdd }) => { //create the AddTodo component and pass in the onAdd function from the prop by destructing
    const [item, setItem] = useState('') //declaring the item variable and setItem function for the useState and setting items default to an empty string
    ...
    const onSubmit = (e) => { //a function literal that takes in the event response
        ...
        onAdd({item}) //invoking the onAdd funtion and passing it the item variable (as an object?) which will add it to our todo list since it is the addTodo function declared in the app.js file
    ...
    return ( //return the component structure we created
        <form onSubmit={onSubmit}> {/**inserts a form and passes our onSubmit function to it */}
            ...
            <input 
                ...
                value={item}  //{/** pass the name variable from the useState to it */}
